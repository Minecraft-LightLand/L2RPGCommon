...
public class LLItems {
	...
	public static final Tab TAB_MAIN = new Tab("material", () -> LLItems.DISPELL_DUST);
	public static final Tab TAB_PROF = new Tab("profession", () -> LLItems.MAGIC_WAND);
	public static final Tab TAB_QUEST = new Tab("generated", () -> LLItems.GEN_ITEM[0][0]);
	...
	static {
		registerEgg("layline_zombie_spawn_egg", 0, 0, () -> LLEntities.ET_LAYLINE_ZOMBIE);
		registerEgg("layline_skeleton_spawn_egg", 0, 0, () -> LLEntities.ET_LAYLINE_SKELETON);
		registerEgg("cursed_knight_spawn_egg", 0, 0, () -> LLEntities.ET_CURSED_KNIGHT);
		registerEgg("cursed_archer_spawn_egg", 0, 0, () -> LLEntities.ET_CURSED_ARCHER);
		registerEgg("cursed_shield_spawn_egg", 0, 0, () -> LLEntities.ET_CURSED_SHIELD);
		registerEgg("potion_slime_spawn_egg", 0, 0, () -> LLEntities.ET_POTION_SLIME);
		registerEgg("stone_slime_spawn_egg", 0, 0, () -> LLEntities.ET_STONE_SLIME);
		registerEgg("vine_slime_spawn_egg", 0, 0, () -> LLEntities.ET_VINE_SLIME);
		registerEgg("carpet_slime_spawn_egg", 0, 0, () -> LLEntities.ET_CARPET_SLIME);
		registerEgg("boss_slime_spawn_egg", 0, 0, () -> LLEntities.ET_BOSS_SLIME);
	}
	...
	static {
		...
		MEDICINE_ARMOR = genArmor("medicine_leather",
				(slot, p) -> new MedicineArmor(Mat.MEDICINE_LEATHER, slot, p), e -> e.model(LLItems::createDoubleLayerModel)
						.color(() -> () -> (stack, val) -> val > 0 ? -1 : ((DyeableLeatherItem) stack.getItem()).getColor(stack)));
		KING_MED_ARMOR = genArmor("king_leather",
				(slot, p) -> new MedicineArmor(Mat.KING_LEATHER, slot, p), e -> e.model(LLItems::createDoubleLayerModel)
						.color(() -> () -> (stack, val) -> val > 0 ? -1 : ((DyeableLeatherItem) stack.getItem()).getColor(stack)));
	}
