package dev.xkmc.l2rpgcommon.init.data.templates;
...
import dev.xkmc.l2rpgcommon.content.common.item.generic.*;
...
public class GenItem {
	...
	public enum Mats {
		...
		Mats(String name, int level,
			...
			Supplier<Ingredient> ing = () -> Ingredient.of(LLItems.MAT_INGOTS[ordinal()].get());
			this.id = name;
		...
		public Item getArmor(EquipmentSlot slot) {
			return LLItems.GEN_ITEM[ordinal()][slot.getIndex()].get();
		}

		public Item getTool(Tools tool) {
			return LLItems.GEN_ITEM[ordinal()][4 + tool.ordinal()].get();
		}

		public Item getIngot() {
			return LLItems.MAT_INGOTS[ordinal()].get();
		}

		public Item getNugget() {
			return LLItems.MAT_NUGGETS[ordinal()].get();
		}
